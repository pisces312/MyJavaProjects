 ...
 * various wave shapes, including sine, cosine, triangle, sawtooth,
...
public class Oscillator extends AudioObject {
     ...
     *            result of choice (choice=0 for example will set a fixed
     *            frequency)
     */
    ...
    //----------------------------------------------
    /**
     * Moves through the Oscillator array (normally forwards but sometimes
     * backwards) by increments set by si (sample increment value). This
     * nextWork
    ...
    public int work(float [] buffer) throws AOException {
        ...
        if (inputs == 2) { //Amp and Freq
            ...
            float [] ampbuf = new float[buffneed];
            this.previous[0].nextWork(ampbuf);
            for (int i = 0; ret < buffer.length; i++) {
    ...
    protected void setSI(double frequency) {
        ...
        this.si = (float)(2 * Math.PI / ((float)sampleRate / frequency));
    }
