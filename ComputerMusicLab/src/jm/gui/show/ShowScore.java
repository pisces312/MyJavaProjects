 ...
 * Where xpos and ypos are intergers specifying the topleft position of the
 * window.
 * This is useful if you want to use DrawScore in conjunction with some other
 * GUI interface
 * which is already positioned in the top left corner of the screen.
 * 
 */

public class ShowScore extends JFrame implements WindowListener, ActionListener {
    /**
     * 
     */
    private static final long serialVersionUID = -4210607221428881325L;
    //attributes
    //	private Panel pan;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == size6)
            resize(6);
        if (e.getSource() == size7)
            resize(7);
        if (e.getSource() == size8)
            resize(8);
        if (e.getSource() == thin)
            sp.getShowArea().setThinNote(2);
        if (e.getSource() == medium)
            sp.getShowArea().setThinNote(1);
        if (e.getSource() == thick)
            sp.getShowArea().setThinNote(0);
    }
