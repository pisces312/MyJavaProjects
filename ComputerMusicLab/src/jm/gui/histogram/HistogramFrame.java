...
public class HistogramFrame extends Frame implements WindowListener, ActionListener, JMC {
     ...
     * 
     */
    private static final long serialVersionUID = 3229164180129366724L;
    //attributes
    ...
    private void setTitle(int dataType) {
        if (dataType == PITCH)
            this.setTitle("jMusic Pitch Histogram: '" + score.getTitle() + "'");
        if (dataType == RHYTHM)
            this.setTitle("jMusic Rhythm Histogram: '" + score.getTitle() + "'");
        if (dataType == DYNAMIC)
            this.setTitle("jMusic Dynamic Histogram: '" + score.getTitle() + "'");
        if (dataType == PAN)
            this.setTitle("jMusic Pan Histogram: '" + score.getTitle() + "'");
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == showPitch)
            changeDataType(PITCH);
        if (e.getSource() == showRhythm)
            changeDataType(RHYTHM);
        if (e.getSource() == showDynamic)
            changeDataType(DYNAMIC);
        if (e.getSource() == showPan)
            changeDataType(PAN);
        if (e.getSource() == open)
            openMIDIFile();
