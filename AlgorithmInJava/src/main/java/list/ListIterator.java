 ...
 * current position in the list. A {@code ListIterator} has no current element;
...
public interface ListIterator<E> extends Iterator<E> {
	 ...
	 * {@code true} if {@link #previous} would return an element rather than
	 * throwing an exception.)
	 * 
	 * @return {@code true} if the list iterator has more elements when
	...
	/**
	 * Removes from the list the last element that was returned by {@link #next}
	 * or {@link #previous} (optional operation). This call can only be made
	 * once per call to {@code next} or {@code previous}. It can be made only if
	 * {@link #add} has not been called after the last call to {@code next} or
	 * {@code previous}.
	 * 
	 ...
	 * only if neither {@link #remove} nor {@link #add} have been called after
	 ...
	 * element is inserted immediately before the element that would be returned
	 * by {@link #next}, if any, and after the element that would be returned by
	 ...
	 * @throws UnsupportedOperationException
	 *             if the {@code add} method is not supported by this list
	 *             iterator
	 * @throws ClassCastException
	 *             if the class of the specified element prevents it from being
	 *             added to this list
